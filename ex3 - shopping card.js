...
function getCartTotalAmount() {
  ...
  for(let i of SHOPPING_CART){
    if(PRODUCTS.some(p=>p.id===i.id)){
      result += PRODUCTS[i.id-1].price * i.quantity;
    }
  }
  return result;
}
...
function addProductToCart(productId) {
  // Write your code here
  let exist=SHOPPING_CART.find(item=>item.id===productId);
  if(exist){
    exist.quantity+=1;
  }else{
...
function removeProductFromCart(productId) {
  ...
  let exist= SHOPPING_CART.find(item=>item.id===productId);
  if(exist){
    if(exist.quantity===1) SHOPPING_CART.splice(SHOPPING_CART.indexOf(exist),1);
    else exist.quantity-=1;
  }else console.log(`There is no porduct with id ${productId} in the cart`);
